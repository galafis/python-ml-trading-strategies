...
def test_calculate_ema():
    ...
    pd.testing.assert_series_equal(
        ema_3.round(4), expected_ema_3.round(4), check_dtype=False
    )
...
def test_calculate_rsi():
    ...
    data = pd.Series(
        [44, 44.5, 45, 45.5, 45, 44.5, 44, 43.5, 44, 44.5, 45, 45.5, 46, 46.5, 47]
    )
...
def test_calculate_macd():
    ...
    np.testing.assert_array_almost_equal(
        hist.dropna().values, (macd - signal).dropna().values
    )
