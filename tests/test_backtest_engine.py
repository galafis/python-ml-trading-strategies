...
def test_generate_signals_from_predictions_proba():
    ...
    signals = BacktestEngine.generate_signals_from_predictions(
        predictions, threshold=0.6
    )
...
def test_run_backtest_multiple_trades():
    ...
    data = pd.DataFrame({"close": [100, 105, 110, 105, 115, 120, 115, 125]})
...
def test_backtest_commission_and_slippage():
    ...
    data = pd.DataFrame({"close": [100, 110]})  # 10% price increase
...
def test_backtest_results_metrics():
    ...
    data = pd.DataFrame({"close": [100, 102, 105, 103, 108, 112, 110, 115]})
...
def test_backtest_win_rate_calculation():
    ...
    data = pd.DataFrame({"close": [100, 110, 105, 115, 110, 120]})
