...
import pytest
from src.models.ml_models import TradingModel, EnsembleModel
...
X_train_data = pd.DataFrame(np.random.rand(100, 10), columns=[f'feature_{i}' for i in range(10)])
y_train_data = pd.Series(np.random.randint(0, 2, 100))
X_test_data = pd.DataFrame(np.random.rand(50, 10), columns=[f'feature_{i}' for i in range(10)])
...
def test_trading_model_save_load(tmp_path):
    ...
    np.testing.assert_array_equal(model.predict(X_test_data), loaded_model.predict(X_test_data))
